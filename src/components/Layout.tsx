  ...
  const navLinks = [
    ...
    { 
      ...
      subItems: [
        ...
        { name: 'Sistemas Gerenciais', path: '/servicos/sistemas-gerenciais' },
      ]
    },
    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-700 ${scrolled ? 'bg-brand-navy/95 backdrop-blur-md border-b border-brand-gold/10 py-2 lg:py-3' : 'bg-transparent py-4 lg:py-8'}`}>
      <div className="max-w-[1600px] mx-auto px-4 lg:px-8 flex justify-between items-center">
        ...
        <div className="hidden lg:flex items-center space-x-10">
          {navLinks.map((link) => (
            <div key={link.name} className="relative group/main">
              <Link 
                to={link.path}
              ...
              {link.subItems && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-72 bg-brand-navy border border-brand-gold/10 opacity-0 invisible group-hover/main:opacity-100 group-hover/main:visible transition-all duration-300 py-4 shadow-2xl backdrop-blur-xl">
                  {link.subItems.map((sub) => (
                    <div key={sub.name} className="relative group/sub">
                      <Link
                        to={sub.path}
                        className="flex items-center justify-between px-8 py-3 text-[11px] uppercase tracking-wider text-brand-white/50 hover:text-brand-gold hover:bg-brand-white/5 transition-all font-bold"
                      >
                        {sub.name}
                        {sub.subItems && <ChevronDown size={14} className="-rotate-90" />}
                      </Link>
                      
                      {sub.subItems && (
                        <div className="absolute top-0 left-full ml-0 w-64 bg-brand-navy border border-brand-gold/10 opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-300 py-4 shadow-2xl backdrop-blur-xl">
                          {sub.subItems.map((child) => (
                            <Link
                              key={child.name}
                              to={child.path}
                              className="block px-8 py-3 text-[10px] uppercase tracking-widest text-brand-white/50 hover:text-brand-gold hover:bg-brand-white/5 transition-all font-bold"
                            >
                              {child.name}
                            </Link>
                          ))}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="lg:hidden absolute top-full left-0 w-full bg-brand-navy border-b border-brand-gold/20 py-8 px-6 shadow-2xl h-[calc(100vh-80px)] overflow-y-auto"
          >
              ...
              {navLinks.map((link) => (
                <div key={link.name}>
                  <Link 
                    ...
                    className="text-sm uppercase tracking-widest text-brand-white font-bold"
                  >
                    {link.name}
                  </Link>
                  {link.subItems && (
                    <div className="mt-4 ml-4 flex flex-col space-y-4 border-l border-brand-gold/20 pl-4">
                      {link.subItems.map((sub) => (
                        <div key={sub.name}>
                          <Link
                            to={sub.path}
                            onClick={() => setIsOpen(false)}
                            className="text-[11px] uppercase tracking-wider text-brand-white/60 font-medium"
                          >
                            {sub.name}
                          </Link>
                          {sub.subItems && (
                            <div className="mt-3 ml-4 flex flex-col space-y-3 border-l border-brand-gold/10 pl-4">
                              {sub.subItems.map((child) => (
                                <Link
                                  key={child.name}
                                  to={child.path}
                                  onClick={() => setIsOpen(false)}
                                  className="text-[10px] uppercase tracking-widest text-brand-white/40 font-light"
                                >
                                  {child.name}
                                </Link>
                              ))}
...
const Footer = () => {
  return (
    <footer className="border-t border-brand-gold/10 pt-12 pb-8">
